...
<section class="ces-table-page">
  <!-- 表格操作按钮 -->
    <section class="ces-handle" style="text-align: right;" v-if='tableHandles.length > 0'>
    ...
    <section class="ces-table">
      <el-table :data='tableData' :size='size' style="width: 100%;height:100%"
        ...
        :default-sort = "defaultSort?defaultSort:{}" @sort-change="changeSort"
        ref="cesTable">
        ...
        <!-- 数据栏 -->
        <el-table-column v-for="item in tableCols" :sortable="item.sortable"
          :key="item.id"
          :prop="item.prop"
          :label="item.label"
          :width="item.width"
          ...
          <template slot-scope="scope" >
            ...
            <span v-if="item.type==='button'" >
              <el-button v-for="btn in item.btnList" :key="(typeof btn.label).toLowerCase()=='string'?btn.label:btn.auth"
                ...
                v-show="!btn.show?true:btn.show(scope.row)"
                :type="btn.type"
                :size="size || btn.size"
                :icon="btn.icon"

                @click="btn.handle && btn.handle(scope.row,scope.$index)">{{(typeof btn.label).toLowerCase()=="string"?btn.label:btn.label(scope.row)}}</el-button>
            </span>
            <!-- 输入框 -->
            <el-input v-if="item.type==='input'" :type="item.inputType" v-model="scope.row[item.prop]" :size="size || btn.size"
              :disabled="item.isDisabled && item.isDisabled(scope.row)"
              @focus="item.focus && item.focus(scope.row)" @change="item.change && item.change(scope.row)"></el-input>
            <!-- 下拉框 -->
            ...
            <el-popover v-if="item.type==='image' && item.popover"
              ...
              <img slot="reference" :src="scope.row[item.prop]" :alt="scope.row[item.prop]" style="height: 60px;width: 60px;border-radius:50%; " :style="item.style?item.style:''">
            </el-popover>
            ...
            <!-- 默认 -->
            <span v-if="!item.type" @click="item.click && item.click(scope.row)"
            ...
            <span  v-if="item.type ==='foreachtext'">
              <p  style='cursor:pointer;color:blue' @click="item.downloadmp4(val)" v-for="(val,index) in scope.row[item.prop]" :key="index">
                  {{val.fileName}}
              </p>
            </span>
    ...
    <!-- 分页 -->
    <section class="ces-pagination" style="margin-top:10px;" v-if='isPagination'>
        <el-pagination
...
export default {
  props:{
    ...
    changeSort:{type:Function, default:()=>function(){}},
  },
  data(){
    return {
    }
  ...
  methods:{
    ...
    handleSizeChange(val) {
      ...
      this.tablePage.currentPage=1//add by zjs20210629
      this.$emit('handleCurrentChange');
    },
    renderHeader(h,obj) {
    ...
    celldblclick(row, column){
      // let _this = this;
      this.$copyText(row[column.property]).then(function () {
        // _this.onCopy()
      }, function () {
        // _this.onError()
      })
